...
csv_output = Path("/home/aboubakr/Descargas/article4/ham10000/pipeline4/efficientnetb7_results.csv")

IMG_SIZE = (600, 600)   # EfficientNetB7 input size
BATCH_SIZE = 8           # Reduce batch size for large images to fit in GPU
EPOCHS = 5
NUM_CLASSES = 7
...
print("✅ EfficientNetB7 compiled successfully")
...
plt.title("Confusion Matrix  EfficientNetB7", fontweight='bold')
plt.tight_layout()
plt.show()
...
plt.title("ROC Curves  EfficientNetB7 (HAM10000)", fontweight='bold')
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.show()
