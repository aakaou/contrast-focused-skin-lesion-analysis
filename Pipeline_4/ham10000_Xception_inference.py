...
from tensorflow.keras.applications.xception import preprocess_input  # Preprocessing function
...
from sklearn.preprocessing import label_binarize           # For one-hot encoding in ROC
...
# =========================
# 7️⃣ Build Xception Model
# =========================
base_model = Xception(
    weights='imagenet',       # Use pretrained ImageNet weights
    include_top=False,        # Exclude top fully-connected layer
    input_shape=(299, 299, 3) # Input shape matches overlay images
)
...
plt.title("Confusion Matrix – Xception", fontweight='bold')
plt.tight_layout()
plt.show()
...
plt.title("ROC Curves – Xception (HAM10000)", fontweight='bold')
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.show()
