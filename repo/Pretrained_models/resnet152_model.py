# ==========================================================
# FULL RESNET152 CLINICAL PIPELINE
...
DATASET_CONFIGS = [
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/resnet152_results"
    },
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/resnet152_results"
    }
]
...
def run_pipeline(cfg):
    ...
    best_model_path = output_dir / "best_resnet152.pth"
    torch.save(best_state, best_model_path)

    history_df = pd.DataFrame(history)
    history_df.to_csv(output_dir / "training_history_resnet152.csv", index=False)
    ...
    pred_df.to_csv(output_dir / "resnet152_predictions.csv", index=False)
    print("Saved: resnet152_predictions.csv")
    ...
    report_df.to_csv(output_dir / "classification_report_resnet152.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_resnet152.png",
        f"{cfg['name']} ResNet152 Confusion Matrix"
    )
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_resnet152.png",
        f"{cfg['name']} ResNet152 ROC Curve"
    )
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_resnet152.png",
        f"{cfg['name']} ResNet152 Precision-Recall Curve"
    )
    ...
    summary_df.to_csv(output_dir / "final_metrics_summary_resnet152.csv", index=False)
    save_json(summary, output_dir / "final_metrics_summary_resnet152.json")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        combined_out = Path("./combined_resnet152_results")
        ensure_dir(combined_out)

        combined_df.to_csv(
            combined_out / "all_datasets_summary_resnet152.csv",
            index=False
        )
        save_json(
            all_results,
            combined_out / "all_datasets_summary_resnet152.json"
        )
