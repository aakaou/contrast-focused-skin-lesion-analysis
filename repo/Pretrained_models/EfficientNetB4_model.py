# ==========================================================
# FULL EFFICIENTNET-B4 CLINICAL PIPELINE
...
DATASET_CONFIGS = [
    ...
    },
    {
        ...
        "patient_col": "lesion_id",
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/efficientb4_results"
    }
]
...
def create_binary_target_if_needed(df, label_col, dataset_type):
    ...
    # HAM10000: map dx to malignant vs benign
...
def run_pipeline(cfg):
    ...
    # Save best model
    best_model_path = output_dir / "best_efficientnet_b4.pth"
    torch.save(best_state, best_model_path)

    # Save history
    history_df = pd.DataFrame(history)
    history_df.to_csv(output_dir / "training_history_efficientb4.csv", index=False)
    ...
    pred_df.to_csv(output_dir / "efficientnetb4_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_efficientb4.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_efficientb4.png",
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_efficientb4.png",
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_efficientb4.png",
    ...
    summary_df.to_csv(output_dir / "final_metrics_summary_efficientb4.csv", index=False)
    save_json(summary, output_dir / "final_metrics_summary_efficientb4.json")

    print("\n==============================")
    print(f"{cfg['name']} EfficientNet-B4 Results")
    print("==============================")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        combined_out = Path("./combined_efficientb4_results")
        ensure_dir(combined_out)

        combined_df.to_csv(combined_out / "all_datasets_summary_efficientb4.csv", index=False)
        save_json(
            all_results,
            combined_out / "all_datasets_summary_efficientb4.json"
        )
