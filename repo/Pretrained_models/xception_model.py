# ==========================================================
# FULL XCEPTION CLINICAL PIPELINE
...
IMG_SIZE = 299          # Xception default size
BATCH_SIZE = 16
...
DATASET_CONFIGS = [
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/xception_results"
    },
    {
        "name": "HAM10000_Xception",
        "dataset_type": "ham10000",
        "img_dir": "/path/to/ham10000/pipeline/unet_overlays_up",
        "csv_file": "/path/to/ham10000/HAM10000_metadata_clean.csv",
        "img_col": "image_id",
        "label_col": "dx",
        "patient_col": "lesion_id",
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/xception_results"
    }
]
...
def create_binary_target_if_needed(df, label_col, dataset_type):
    ...
    # ISIC 2024 typically has target already binary (0/1)
    if not pd.api.types.is_numeric_dtype(df[label_col]):
        # HAM10000: map dx to malignant vs benign if needed
...
def save_roc_curve(y_true, y_probs, out_path, title):
    ...
    plt.plot(fpr_b, tpr_b, linewidth=2, label=f"Benign (AUC={auc_b:.4f})")
    plt.plot(fpr_m, tpr_m, linewidth=2, label=f"Malignant (AUC={auc_m:.4f})")
    plt.plot([0, 1], [0, 1], "--")
...
def run_pipeline(cfg):
    ...
    best_model_path = output_dir / "best_xception.pth"
    torch.save(best_state, best_model_path)

    pd.DataFrame(history).to_csv(
        output_dir / "training_history_xception.csv",
        index=False
    )
    ...
    model.eval()
    print("Best Xception loaded ✔")

    # ==========================================================
    # PART 3/3 – EVALUATION + REPORTS + PLOTS
    # ==========================================================
    print("\nRunning evaluation...")
    ...
    pred_df.to_csv(output_dir / "xception_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_xception.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_xception.png",
        f"{cfg['name']} Xception Confusion Matrix"
    )

    # =========================
    # ROC CURVE (Benign/Malignant style)
    # =========================
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_xception.png",
        f"{cfg['name']} Xception ROC Curve"
    )
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_xception.png",
        f"{cfg['name']} Xception Precision-Recall Curve"
    )
    ...
    pd.DataFrame([summary]).to_csv(
        output_dir / "final_metrics_summary_xception.csv",
        index=False
    )
    save_json(summary, output_dir / "final_metrics_summary_xception.json")

    print("\n==============================")
    print(f"{cfg['name']} Results (Xception)")
    print("==============================")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        combined_out = Path("./combined_xception_results")
        ensure_dir(combined_out)

        combined_df = pd.DataFrame(all_results)
        combined_df.to_csv(
            combined_out / "all_datasets_summary_xception.csv",
            index=False
        )
        save_json(
            all_results,
            combined_out / "all_datasets_summary_xception.json"
        )
