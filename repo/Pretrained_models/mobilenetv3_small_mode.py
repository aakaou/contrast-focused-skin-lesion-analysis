# ==========================================================
# FULL MOBILENETV3_SMALL CLINICAL PIPELINE
...
DATASET_CONFIGS = [
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/mobilenetv3_small_results"
    },
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/mobilenetv3_small_results"
    }
]
...
def build_model():
    ...
    # Replace classifier (final linear layer)
...
def create_binary_target_if_needed(df, label_col, dataset_type):
    ...
    if not pd.api.types.is_numeric_dtype(df[label_col]):
        if dataset_type == "ham10000":
            # example mapping; adjust if you use another malignant definition
            mapping = {
                "mel": 1,
                "bcc": 1,
                "akiec": 1,
...
def run_pipeline(cfg):
    ...
    history_df.to_csv(output_dir / "training_history_mobilenetv3_small.csv", index=False)
    ...
    print("Best MobileNetV3 Small loaded ✔")
    ...
    pred_df.to_csv(output_dir / "mobilenetv3_small_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_mobilenetv3_small.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_mobilenetv3_small.png",
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_mobilenetv3_small.png",
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_mobilenetv3_small.png",
    ...
    summary_df.to_csv(output_dir / "final_metrics_summary_mobilenetv3_small.csv", index=False)
    save_json(summary, output_dir / "final_metrics_summary_mobilenetv3_small.json")

    print("\n==============================")
    print(f"{cfg['name']} MobileNetV3 Small Results")
    print("==============================")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        combined_out = Path("./combined_mobilenetv3_small_results")
        ensure_dir(combined_out)

        combined_df.to_csv(
            combined_out / "all_datasets_summary_mobilenetv3_small.csv",
            index=False
        )
        save_json(
            all_results,
            combined_out / "all_datasets_summary_mobilenetv3_small.json"
        )
