...
DATASET_CONFIGS = [
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/vgg16_results"
    },
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/vgg16_results"
    }
]
...
def run_pipeline(cfg):
    ...
    best_model_path = output_dir / "best_vgg16.pth"
    torch.save(best_state, best_model_path)

    pd.DataFrame(history).to_csv(
        output_dir / "training_history_vgg16.csv",
        index=False
    )
    ...
    pred_df.to_csv(output_dir / "vgg16_predictions.csv", index=False)
    print("Saved: vgg16_predictions.csv")
    ...
    report_df.to_csv(output_dir / "classification_report_vgg16.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_vgg16.png",
        f"{cfg['name']} VGG16 Confusion Matrix"
    )
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_vgg16.png",
        f"{cfg['name']} VGG16 ROC Curve"
    )
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_vgg16.png",
        f"{cfg['name']} VGG16 Precision-Recall Curve"
    )
    ...
    pd.DataFrame([summary]).to_csv(
        output_dir / "final_metrics_summary_vgg16.csv",
        index=False
    )
    save_json(summary, output_dir / "final_metrics_summary_vgg16.json")
    ...
    print("\nSaved files:")
    print("best_vgg16.pth")
    print("vgg16_predictions.csv")
    print("classification_report_vgg16.csv")
    print("confusion_matrix_vgg16.png")
    print("roc_auc_vgg16.png")
    print("precision_recall_vgg16.png")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        combined_out = Path("./combined_vgg16_results")
        ensure_dir(combined_out)

        combined_df = pd.DataFrame(all_results)
        combined_df.to_csv(
            combined_out / "all_datasets_summary_vgg16.csv",
            index=False
        )
        save_json(
            all_results,
            combined_out / "all_datasets_summary_vgg16.json"
        )
