# ==========================================================
# FULL EFFICIENTNET-B3 CLINICAL PIPELINE
...
# ==========================================================
# IMPORTANT:
# - Update paths and column names to match your actual files.
# - ISIC-style:
#     img_col     = "isic_id"
...
DATASET_CONFIGS = [
    {
        ...
        "patient_col": "patient_id",
        "file_ext": ".jpg",
        "output_dir": "/path/to/efficientb3_results"
    },
    {
        ...
        "patient_col": "lesion_id",
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/efficientb3_results"
    }
]
...
def create_binary_target_if_needed(df, label_col, dataset_type):
    ...
    if not pd.api.types.is_numeric_dtype(df[label_col]):
        if dataset_type == "ham10000":
            # Example HAM10000 mapping: malignant vs benign
            mapping = {
                "mel": 1,
                "bcc": 1,
                "akiec": 1,
...
def run_pipeline(cfg):
    ...
    # Save best model
    best_model_path = output_dir / "best_efficientnet_b3.pth"
    torch.save(best_state, best_model_path)

    # Save history
    history_df = pd.DataFrame(history)
    history_df.to_csv(output_dir / "training_history_efficientb3.csv", index=False)
    ...
    pred_df.to_csv(output_dir / "efficientnetb3_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_efficientb3.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_efficientb3.png",
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_efficientb3.png",
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_efficientb3.png",
    ...
    summary_df.to_csv(output_dir / "final_metrics_summary_efficientb3.csv", index=False)
    save_json(summary, output_dir / "final_metrics_summary_efficientb3.json")

    print("\n==============================")
    print(f"{cfg['name']} EfficientNet-B3 Results")
    print("==============================")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        combined_out = Path("./combined_efficientb3_results")
        ensure_dir(combined_out)

        combined_df.to_csv(combined_out / "all_datasets_summary_efficientb3.csv", index=False)
        save_json(
            all_results,
            combined_out / "all_datasets_summary_efficientb3.json"
        )
