# ==========================================================
# FULL INCEPTIONRESNETV2 CLINICAL PIPELINE
...
IMG_SIZE = 299          # InceptionResNetV2 default
BATCH_SIZE = 16
...
DATASET_CONFIGS = [
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/inceptionresnetv2_results"
    },
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/inceptionresnetv2_results"
    }
]
...
def build_model():
    # timm inception_resnet_v2, binary classifier
...
def create_binary_target_if_needed(df, label_col, dataset_type):
    ...
    if not pd.api.types.is_numeric_dtype(df[label_col]):
        if dataset_type == "ham10000":
            # example mapping; adapt if your labels differ
            mapping = {
                "mel": 1,
                "bcc": 1,
                "akiec": 1,
...
def run_pipeline(cfg):
    ...
    best_model_path = output_dir / "best_inceptionresnetv2.pth"
    torch.save(best_state, best_model_path)

    history_df = pd.DataFrame(history)
    history_df.to_csv(output_dir / "training_history_inceptionresnetv2.csv", index=False)
    ...
    print("Best InceptionResNetV2 loaded ✔")
    ...
    pred_df.to_csv(output_dir / "inceptionresnetv2_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_inceptionresnetv2.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_inceptionresnetv2.png",
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_inceptionresnetv2.png",
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_inceptionresnetv2.png",
    ...
    summary_df.to_csv(output_dir / "final_metrics_summary_inceptionresnetv2.csv", index=False)
    save_json(summary, output_dir / "final_metrics_summary_inceptionresnetv2.json")

    print("\n==============================")
    print(f"{cfg['name']} InceptionResNetV2 Results")
    print("==============================")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        combined_out = Path("./combined_inceptionresnetv2_results")
        ensure_dir(combined_out)

        combined_df.to_csv(
            combined_out / "all_datasets_summary_inceptionresnetv2.csv",
            index=False
        )
        save_json(
            all_results,
            combined_out / "all_datasets_summary_inceptionresnetv2.json"
        )
