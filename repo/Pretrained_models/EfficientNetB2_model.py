# ==========================================================
# FULL EFFICIENTNET-B2 CLINICAL PIPELINE
...
IMG_SIZE = 224
BATCH_SIZE = 16
EPOCHS = 10          # change if you want more training
LR = 1e-4
NUM_WORKERS = 4
RANDOM_STATE = 42
TEST_SIZE = 0.20
...
# ==========================================================
# IMPORTANT:
# - Update paths and column names to match your files.
# - ISIC-style:
#     img_col     = "isic_id"
...
DATASET_CONFIGS = [
    ...
    },
    {
        ...
        "img_dir": "/path/to/ham10000/unet_overlays_up",
        "csv_file": "/path/to/ham10000/HAM10000_metadata_clean.csv",
        "img_col": "image_id",
        "label_col": "dx",
        "patient_col": "lesion_id",
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/efficientb2_results"
    }
]

# ==========================================================
# TRANSFORMS
# EfficientNet-B2 pretrained weights use ImageNet normalization.
# ==========================================================
...
# ==========================================================
# MODEL
# EfficientNet-B2 via torchvision pretrained weights.
# ==========================================================
...
def create_binary_target_if_needed(df, label_col, dataset_type):
    ...
    # If label is not numeric, map for HAM10000
    if not pd.api.types.is_numeric_dtype(df[label_col]):
        if dataset_type == "ham10000":
            # Example malignant vs benign mapping
            mapping = {
                "mel": 1,
                "bcc": 1,
                "akiec": 1,
...
def evaluate_model(model, loader):
    ...
    return y_true, y_probs, image_ids
...
def save_roc_curve(y_true, y_probs, out_path, title):
    ...
    plt.plot(fpr_b, tpr_b, label=f"Benign (AUC={auc_b:.4f})")
    plt.plot(fpr_m, tpr_m, label=f"Malignant (AUC={auc_m:.4f})")
    plt.plot([0, 1], [0, 1], "--")
    plt.title(title)
...
# ==========================================================
# MAIN RUN FUNCTION (ONE DATASET CONFIG)
# ==========================================================
...
def run_pipeline(cfg):
    ...
    # Save best model
    best_model_path = output_dir / "best_efficientnet_b2.pth"
    torch.save(best_state, best_model_path)

    # Save training history
    history_df = pd.DataFrame(history)
    history_df.to_csv(output_dir / "training_history_efficientb2.csv", index=False)
    ...
    print("Best EfficientNet-B2 loaded ✔")
    ...
    pred_df.to_csv(output_dir / "efficientnetb2_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_efficientb2.csv")
    ...
    save_confusion_matrix(
        cm,
        output_dir / "confusion_matrix_efficientb2.png",
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_efficientb2.png",
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_efficientb2.png",
    ...
    summary_df.to_csv(output_dir / "final_metrics_summary_efficientb2.csv", index=False)
    save_json(summary, output_dir / "final_metrics_summary_efficientb2.json")

    print("\n==============================")
    print(f"{cfg['name']} EfficientNet-B2 Results")
    print("==============================")
    print(f"Balanced Accuracy : {bal_acc:.4f}")
    print(f"ROC-AUC Malignant : {auc_m:.4f}")
    print(f"ROC-AUC Benign    : {auc_b:.4f}")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        ...
        combined_out = Path("./combined_efficientb2_results")
        ensure_dir(combined_out)

        combined_df.to_csv(combined_out / "all_datasets_summary_efficientb2.csv", index=False)
        save_json(
            all_results,
            combined_out / "all_datasets_summary_efficientb2.json"
        )
