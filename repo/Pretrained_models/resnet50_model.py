# ==========================================================
# FULL RESNET50 CLINICAL PIPELINE
...
DATASET_CONFIGS = [
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/resnet50_results"
    },
    {
        ...
        "file_ext": ".jpg",
        "output_dir": "/path/to/ham10000/resnet50_results"
    }
]
...
def run_pipeline(cfg):
    ...
    best_model_path = output_dir / "best_resnet50.pth"
    torch.save(best_state, best_model_path)

    pd.DataFrame(history).to_csv(
        output_dir / "training_history_resnet50.csv",
        index=False
    )
    ...
    pred_df.to_csv(output_dir / "resnet50_predictions.csv", index=False)
    ...
    report_df.to_csv(output_dir / "classification_report_resnet50.csv")
    ...
    save_confusion_matrix(
        ...
        f"{cfg['name']} ResNet50 Confusion Matrix"
    )
    ...
    auc_m, auc_b = save_roc_curve(
        y_true,
        y_probs,
        output_dir / "roc_auc_resnet50.png",
        f"{cfg['name']} ResNet50 ROC Curve"
    )
    ...
    save_pr_curve(
        ...
        precision,
        pr_auc,
        output_dir / "precision_recall_resnet50.png",
        f"{cfg['name']} ResNet50 Precision-Recall Curve"
    )
    ...
    pd.DataFrame([summary]).to_csv(
        output_dir / "final_metrics_summary_resnet50.csv",
        index=False
    )
    save_json(summary, output_dir / "final_metrics_summary_resnet50.json")
    ...
    print("resnet50_predictions.csv")
    print("classification_report_resnet50.csv")
    print("confusion_matrix_resnet50.png")
    print("roc_auc_resnet50.png")
    print("precision_recall_resnet50.png")
...
if __name__ == "__main__":
    ...
    if len(all_results) > 0:
        combined_out = Path("./combined_resnet50_results")
        ensure_dir(combined_out)

        combined_df = pd.DataFrame(all_results)
        combined_df.to_csv(
            combined_out / "all_datasets_summary_resnet50.csv",
            index=False
        )
        save_json(
            all_results,
            combined_out / "all_datasets_summary_resnet50.json"
        )
